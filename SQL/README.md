...
concrete system throughout. Every level runs real SQL against a real local database —
nothing here is theoretical pseudo-SQL. Where a claim can be measured (a timing
...
database itself enforces, not your application code. You query with **SQL**, a
...
- It is the closest thing the industry has to a **default answer** to "which
  ...
  workloads, and the one most interviewers assume when they say "SQL database."
...
- Its internals (MVCC, WAL, B-tree indexes) are the same internals `CSFundamentals/03_databases_deep_dive.md`
...
transactions, indexes, injection, migrations, pooling) applies to MySQL, SQL Server,
...
Every level's SQL is self-contained: each one drops and recreates its own tables (or
...
| 09 | [Transactions and isolation levels](09_transactions_and_isolation_levels.md) | ACID precisely, a real lost-update race, and how to prevent it in Postgres |
...
| 14 | [Being a client: capstone](14_being_a_client_capstone.md) | A small CRUD service with pooling, retries, and timeouts, tying it all together |
