...
Back in Phase 3 (Day 83), we learned basic RAG. Basic RAG is "blind". It takes a user query, pulls the top 5 chunks from a Vector DB, and forces the LLM to write an answer. 
But what if the Vector DB returns 5 chunks of irrelevant garbage? The LLM will still try to answer, resulting in massive hallucinations.

Today, we upgrade RAG to **Agentic RAG**. We give the RAG pipeline a brain, allowing it to route queries, grade documents, and dynamically fallback to the open web!
...
- If the prompt is *"What were my sales yesterday?"* $\rightarrow$ Route to **SQL Agent**.
- If the prompt is *"How do I install the software?"* $\rightarrow$ Route to **Vector DB (Docs)**.
- If the prompt is *"Who won the game last night?"* $\rightarrow$ Route to **Web Search API**.

### 2. Corrective RAG (CRAG)
This is a famous LangGraph architecture that fixes "blind" retrieval.
1. **Retrieve:** Pull 5 documents from the Vector DB.
2. **Grade:** An Evaluator LLM reads the 5 documents. If a document does not actually answer the user's question, the Evaluator deletes it.
...
### 3. Self-RAG
Instead of using complex LangGraph routing, Self-RAG uses a specifically fine-tuned LLM. During generation, the LLM emits special tokens:
- `[Retrieve]`: The LLM pauses itself and tells the system to fetch data.
- `[IsRelevant]`: The LLM grades the data it just received.
- `[IsSupported]`: The LLM verifies that the sentence it is currently writing is actually backed by the retrieved data!

---

## 🕒 HOUR 2: GUIDED CODE-ALONG (THE APPLIED WAY)

Let's build a conceptual **CRAG (Corrective RAG) Pipeline**! We will simulate the grading node and the Web Search fallback.
...
1. **The Grader Node:** This is the most important part of CRAG. By forcing an LLM to explicitly output `PASS` or `FAIL`, we prevent garbage data from ever reaching the final Generator. This virtually eliminates RAG hallucinations.
2. **Self-Healing RAG:** In Scenario 2, the basic RAG pipeline would have crashed or lied. Our Agentic RAG pipeline realized it didn't know the answer, dynamically pivoted to a Web Search tool, and successfully healed the execution path!
...
3. If the user says: *"How many users signed up today?"*, route to a Text-to-SQL node.
...
**The Question:**
*"Design a RAG agent for a hospital. It must query patient records (SQL), clinical guidelines (PDFs), and drug interactions (REST API). Discuss the routing architecture, access control (HIPAA), and citation mechanisms."*

#### 📝 Strong Hire Rubric (Evaluate your answer against this):
A "Strong Hire" candidate must articulate the following points clearly:

1. **The Intent Router:** 
   - Propose an LLM Router that strictly categorizes the intent. If `intent == patient_data`, it triggers the Text-to-SQL Agent.
2. **Access Control (HIPAA):**
   - The LLM MUST NOT have raw database credentials. Use the **MCP (Model Context Protocol)** Gateway pattern from Day 127. The MCP Gateway intercepts the query, injects the Doctor's `user_id` into the SQL `WHERE` clause (Row-Level Security), ensuring the doctor can only query their own patients.
3. **Strict Citations:**
   - In healthcare, hallucinations are fatal. The Generator node must be prompted to output strict citations using `[Document_ID]`. 
   - Post-processing: A Regex script verifies that every sentence generated by the LLM contains a citation. If a sentence lacks a citation, the system deletes the sentence before showing it to the doctor!

---
**Task for the end of the day:** Commit your code to Git. 

We have combined RAG with Agents. 
