...
Language models have profound amnesia. Every time you start a Python script, the LLM forgets everything about you. 
...
### 1. The Taxonomy of AI Memory
Human brains have different storage mechanisms. AI needs them too.
- **Working Memory (RAM):** The LLM's current Context Window (e.g., 128k tokens). It is instantly accessible but strictly limited by the API. If it overflows, the Agent crashes.
...
**Memory Consolidation** is the AI equivalent of sleeping. 
...
1. **Context Enrichment:** By querying the `semantic_db` *before* hitting the LLM API, the Agent injected the user's peanut allergy into the prompt. The user didn't have to remind the Agent! This creates a magical User Experience.
...
3. It passes the buffer to an LLM with the prompt: *"Extract 3 permanent facts about the user from this transcript. Format as JSON."*
...
**The Question:**
*"Design a memory system for a personal AI assistant used by 10 Million users. Each user has years of conversation history. Discuss storage, retrieval latency, privacy, and the cold-start problem."*
