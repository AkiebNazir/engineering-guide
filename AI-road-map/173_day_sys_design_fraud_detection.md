...
Because we only have 50ms, we cannot run a massive LLM or a 100-layer Neural Network. We use a cascade:
...
- **Layer 2 (Gradient Boosting):** Takes 10ms. A lightning-fast ML model (like XGBoost or LightGBM). It looks at complex non-linear patterns (e.g., *User is in New York, but their phone's GPS is in London*). 
- **Layer 3 (Asynchronous Graph / LLM):** Takes 2,000ms. This runs *after* the transaction is approved. It analyzes massive graph networks to find coordinated crime rings. If it finds fraud, it flags the account to be blocked before the *next* swipe.
...
To the ML model, the $2,000 transaction looks normal *unless* it knows about the previous two swipes!
We use a **Streaming Feature Store** (like Redis + Apache Flink). When a swipe occurs, Flink updates the `user_spend_last_10_min` counter in Redis in under 5ms. The ML model pulls this feature from Redis to make its prediction.
...
1. **The Architecture:** Draw the API Gateway routing to a highly available microservice. Explain the Rule Engine $\rightarrow$ ML Model cascade to meet latency constraints.
2. **Feature Computation:** Differentiate between Batch Features (calculated nightly in Snowflake, like `average_monthly_spend`) and Streaming Features (calculated in real-time via Kafka/Flink, like `spend_last_5_minutes`). Both are served to the ML model via a Redis Feature Store.
...
4. **Human-in-the-Loop & Drift:** Fraudsters adapt daily. When the ML model flags a transaction with medium confidence (e.g., 60-80%), it routes to a human reviewer. The human's decision acts as Ground Truth data to continuously retrain the ML model (mitigating Concept Drift).
...
Tomorrow, in **Day 174**, we design an AI system you likely use every day: **An Autonomous Coding Assistant (like GitHub Copilot)**. How does it know what code to suggest when you type a comment?
