...
4.  **Uniform Interface:** Resources are manipulated through representations (like JSON), and standard HTTP verbs are used. It has four sub-rules:
    *   *Identification of resources* (every thing has a URI).
    *   *Manipulation through representations* (send/receive JSON, not the object itself).
...
5.  **Layered System:** The client cannot tell whether it talks to the origin server or to a proxy, CDN, or gateway in between.
...
| IDs are opaque | `/orders/8f14e45f` (UUID) | Sequential IDs that leak volume and invite scraping |

**Modelling actions that do not fit CRUD.** Make the action a resource, or a sub-resource:
...
## Methods, Status Codes and CRUD Mapping
...
**JSON Merge Patch (RFC 7396)**, `Content-Type: application/merge-patch+json`. Send a fragment; `null` means "delete this field".
...
**JSON Patch (RFC 6902)**, `Content-Type: application/json-patch+json`. An ordered list of operations.
...
Never hold an HTTP request open for minutes. Return `202 Accepted` and a status resource:
...
Caching is REST's superpower. CDNs, browsers and proxies handle `GET` caching for free. It is the main reason to prefer REST for public read-heavy APIs.
...
| :---: | :--- | :--- |
| **0** | One URL, one verb (`POST`); RPC tunnelled over HTTP. | `POST /api {"action":"getUser","id":1}` |
| **1** | **Resources**: separate URLs per thing. | `POST /users/1` |
| **2** | **HTTP verbs + status codes** used correctly. | `GET /users/1`, `DELETE /users/1` -> `204` |
| **3** | **Hypermedia (HATEOAS)**: responses link to next actions. | see below |

Most production "REST" APIs are **level 2**, and that is fine.
...
| **Best for** | Public, CRUD-like APIs | Internal, action-heavy APIs (see gRPC) |
...
5.  **Leaking the database schema.** `password_hash` in a response, or binding request JSON straight to a DB model (mass assignment).
...
| :---: | :--- | :--- |
| 1 | `01_crud_stdlib.py` | An HTTP server from scratch: routing, status codes, `Location`, `405 Allow`, idempotent `DELETE` |
| 2 | `02_fastapi_validation_openapi.py` | Pydantic validation, `422` vs `409`, `response_model` hiding secrets, generated OpenAPI |
| 3 | `03_jwt_auth_and_scopes.py` | JWT built by hand, `401` vs `403` vs `404`, scope dependencies, BOLA, forged / expired / `alg=none` tokens |
