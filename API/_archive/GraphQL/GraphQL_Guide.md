...
### Deep Dive
**GraphQL** is a query language for APIs and a runtime for fulfilling those queries. Instead of relying on multiple REST endpoints, GraphQL exposes a single endpoint (usually `POST /graphql`) and allows the client to dictate exactly what shape of data it needs.
...
- **Request/Response Lifecycle**:
  1. Client sends a POST request with a GraphQL query (string) and variables (JSON).
  2. Server parses and validates the query against the Schema.
  3. The Execution Engine traverses the query tree (AST) and runs resolvers concurrently where possible.
  4. The engine constructs a single JSON response mirroring the exact structure requested by the client.
...
- **Caching Complexity**: HTTP caching (CDN layer) breaks down because everything is a POST request to `/graphql`. You must rely on complex client-side caching (e.g., Apollo Client caching via `__typename` and `id`).
...
**Anti-Patterns:**
- **Simple CRUD Services**: Overkill if the client just needs a simple list of identical objects.
...
**System Design Fit:** When a query requests `Users -> Posts`, DataLoaders batch the post requests to the DB to prevent hundreds of individual SQL queries.
