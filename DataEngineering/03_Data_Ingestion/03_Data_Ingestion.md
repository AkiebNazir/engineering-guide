...
```arch
node a1 "Source" at 0,0 icon=db color=slate
node b1 "Processing Engine" at 1,0 icon=process color=amber
node c1 "Clean Data" at 2,0 shape=card color=teal
node d1 "Data Warehouse" at 3,0 icon=db color=purple
a1 -> b1 : "Extract"
b1 -> c1 : "Transform"
c1 -> d1 : "Load"
node a2 "Source" at 0,1 icon=db color=slate
node b2 "Raw Data Storage" at 1,1 shape=card color=amber
node c2 "Data Warehouse" at 2,1 icon=db color=purple
node d2 "Structured Models" at 3,1 shape=card color=teal
a2 -> b2 : "Extract"
b2 -> c2 : "Load"
c2 -> d2 : "Transform"
```
...
```arch
node a "App DB" at 0,0 icon=db color=slate
node b "Batch Ingestion Job" at 1,0 icon=process color=teal
node c "Data Lake" at 2,0 icon=cloud color=purple
node d "Web Servers" at 0,1 icon=server color=slate
node e "Kafka / PubSub" at 1,1 icon=process color=amber
a -> b : "Nightly CRON"
b -> c
d -> e : "Continuous Events"
e -> c : "Streaming Job"
```
...
```arch
node app "App" at 0,0 icon=users color=slate
node db "PostgreSQL" at 1,0 icon=db color=blue
node wal "Write-Ahead Log" at 2,0 icon=doc color=purple
node deb "Debezium" at 3,0 icon=process color=teal
node kaf "Kafka" at 4,0 icon=process color=amber
app -> db : "UPDATE users"
db -> wal : "Record tx"
deb -> wal : "Read log"
deb -> kaf : "Publish event"
```
...
```arch
node job "Ingestion Job" at 0,0 icon=process color=blue
node api "SaaS API" at 1,0 icon=gateway color=teal
job -> api : "GET"
api -> job : "JSON"
node saas "SaaS App" at 0,1 icon=gateway color=amber
node gw "API Gateway" at 1,1 icon=gateway color=teal
node pub "Message Queue" at 2,1 icon=process color=purple
node stor "Data Lake" at 3,1 icon=cloud color=blue
saas -> gw : "POST Event"
gw -> pub
pub -> stor
```
...
```arch
node a "Ingestion Job Retry" at 0,0 icon=process color=blue
node b "Does record exist?" at 1,0 shape=card color=amber
node c "UPDATE record" at 2,0 shape=text color=teal
node d "INSERT record" at 2,1 shape=text color=green
node e "Data Warehouse" at 3,0 icon=db color=purple
a -> b
b -> c : "Yes"
b -> d : "No"
c -> e
