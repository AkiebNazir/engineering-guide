...
## 3. The "Hello World": Local CPU Inference
...
- `Q4_K_M` (4-bit Medium): **The Gold Standard.** It provides the absolute best balance between speed, tiny RAM footprint, and high accuracy. Almost everyone uses this.
- `Q2_K` (2-bit): Maximum compression. The model fits in tiny RAM, but it will suffer severe brain damage and hallucinate heavily.
...
- `n_ctx` (int - Context Window): Default is `512`.
  - *Effect of increasing (e.g., 4096):* It allows you to paste massive documents into the prompt. However, because it runs on the CPU, allocating a 4096-token KV cache will eat massive amounts of standard RAM and significantly slow down the time it takes to process the first word (Time-To-First-Token). Always set this to the lowest number you can safely get away with.
- `n_threads` (int): How many CPU cores to use.
  - *Effect:* If you have an 8-core CPU, setting this to `8` utilizes 100% of your processor. Setting it to `4` leaves half your processor free for watching YouTube while the model generates. If you don't set this, it defaults to `max_cores`, which might overheat laptops.
- `n_gpu_layers` (int): The Hybrid Superpower.
  - *Effect:* If you have a weak GPU with only 4GB of VRAM, you cannot fit an 8GB model entirely on it. If you set `n_gpu_layers=15`, Llama.cpp will push exactly 15 neural network layers onto your GPU, and leave the remaining 17 layers on your CPU. This **Hybrid Processing** squeezes maximum speed out of whatever junk hardware you have available! To push the entire model to the GPU, set it to `-1`.
...
*Interviewer:* "A developer downloaded a 16GB `.safetensors` model file from Hugging Face and wrote a PyTorch script to run it on CPU. It is generating 1 word per second. Why is it so slow, and why would GGUF fix it?"

*Answer:* "PyTorch is fundamentally designed around dynamic computation graphs and heavily optimized for NVIDIA CUDA architectures. When forced to run on CPU, PyTorch's matrix multiplication kernels are inefficient, and 16-bit floats (`.safetensors`) strangle the CPU's memory bandwidth. 
