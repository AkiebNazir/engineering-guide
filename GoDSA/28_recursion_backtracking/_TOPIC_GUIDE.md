...
- [ ] Return `(a, b)` from a tree recursion for House Robber III, Split <abbr title="Binary Search Tree. A node-based binary tree data structure where the left subtree has smaller values and the right subtree has larger values than the parent node.">BST</abbr>, <abbr title="Lowest Common Ancestor. In a tree or directed acyclic graph, the lowest node that has both given nodes as descendants.">LCA</abbr> of Deepest Leaves and Cameras — and give the three cameras states
