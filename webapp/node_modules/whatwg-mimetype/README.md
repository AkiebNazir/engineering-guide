...
The `MIMETypeParameters` class, instances of which are returned by `mimeType.parameters`, has equivalent surface <abbr title="Application Programming Interface">API</abbr> to a [JavaScript `Map`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map).
...
* **`options.contentTypeHeader`** (`string`): The Content-Type header value, for <abbr title="Hypertext Transfer Protocol - The foundation of data communication for the World Wide Web, operating on a client-server model.">HTTP</abbr> resources.
* **`options.providedType`** (`string`): The MIME type from the filesystem or another protocol (like FTP), for non-<abbr title="Hypertext Transfer Protocol - The foundation of data communication for the World Wide Web, operating on a client-server model.">HTTP</abbr> resources.
