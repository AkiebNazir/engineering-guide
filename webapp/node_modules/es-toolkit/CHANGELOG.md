...
- Use the compatibility layer `es-toolkit/compat` in legacy <abbr title="Content Delivery Network - A geographically distributed network of proxy servers and their data centers used to deliver content with low latency.">CDN</abbr> builds.
...
We're introducing `es-toolkit/compat`, a new module designed as a drop-in replacement for lodash. It replicates lodash's <abbr title="Application Programming Interface">API</abbr>, making it easier to switch between the two libraries.
