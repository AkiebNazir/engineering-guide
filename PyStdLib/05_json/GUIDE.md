...
`json` converts Python values to/from the JSON text format: `dict`<->object,
`list`/`tuple`<->array, `str`<->string, `int`/`float`<->number, `True`/`False`/`None`
<->`true`/`false`/`null`. It is the default choice for config files, HTTP payloads,
...
- Need schema validation, type coercion, or nested model objects, not just
  ...
  builds on top of exactly this module for its own JSON mode).
...
- Need a config file a human hand-edits with comments -> `tomllib` (see
  `PyEngineering/08_config_loader`) reads TOML, which allows comments; JSON does not.
...
| `NaN`/`Infinity` are non-standard | Python's `json` accepts and emits `NaN`, `Infinity`, `-Infinity` by default — valid Python, **invalid** strict JSON per RFC 8259. |
...
Level 7 covers a lifecycle concern: writing a JSON file atomically so a crash
