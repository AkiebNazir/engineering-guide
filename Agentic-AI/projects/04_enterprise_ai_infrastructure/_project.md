...
2. **Model Context Protocol (<abbr title="Model Context Protocol">MCP</abbr>)**: Agents need standard ways to talk to your enterprise DB. Write an <abbr title="Model Context Protocol">MCP</abbr> server using Python (`fastmcp` or `@modelcontextprotocol/sdk`) that exposes a `query_employee_db` tool.
3. **Production Grade**: Include proper typing, logging, and environment variable configurations.

## The Challenge
Implement the architecture. Try to figure out how to start a vLLM server in Python and how to define a tool using the <abbr title="Model Context Protocol">MCP</abbr> standard. 
