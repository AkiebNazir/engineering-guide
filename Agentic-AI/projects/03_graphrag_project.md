...
## Objective
Build a miniature GraphRAG system. The system will use an <abbr title="Large Language Model">LLM</abbr> to read a paragraph of text, extract entities and relationships in a structured format (JSON), build a simple in-memory graph (nodes and edges), and then traverse that graph to answer questions.

## Design Problem

1. **Entity Extraction (Information Extraction)**
   - Pass a text snippet to the <abbr title="Large Language Model">LLM</abbr>.
   - Use prompting (and optionally JSON mode or constrained decoding if your local model supports it) to force the <abbr title="Large Language Model">LLM</abbr> to output a list of relationships in the format:
