...
- 720p30 video that degrades gracefully to audio-only. Uplinks range from 1 to 100 Mbps and 1 to 5% packet loss is common. Some networks block <abbr title="User Datagram Protocol - A simple, connectionless communication protocol that allows for sending messages with minimal overhead but no delivery guarantees.">UDP</abbr>.
