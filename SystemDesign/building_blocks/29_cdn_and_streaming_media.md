...
| Steering | DNS returns a POP by resolver location or measured latency. Anycast announces one IP from every POP and BGP picks the path ([27](27_multi_region_and_global_traffic.md)). | DNS is as coarse as the resolver and as slow as its TTL. Anycast follows BGP, not latency, and a route flap can break a long TCP flow. Large CDNs combine both. |
| TLS and DDoS | TLS terminates at the edge, near the viewer, with pooled persistent connections to the origin. Aggregate edge capacity absorbs volumetric attacks ([02](02_networking.md)). | The edge sees plaintext, so protect its keys ([14](14_security.md)). |
...
HLS (RFC 8216) and MPEG-DASH describe the same idea, a manifest listing renditions plus a sequence of short segment files. CMAF (fragmented MP4) lets one set of segments serve both, so the CDN caches one copy, not two. Segment duration is a five-way trade:
...
**QoE metrics:** start-up time, rebuffer ratio (stall time ÷ play time), average bitrate or VMAF, and number and size of switches. They conflict (a higher rung raises quality and rebuffer risk), so QoE is a weighted combination, as in the MPC paper. **Start-up budget** (assumption: 30 ms RTT, 10 Mbps, 2 s segments, first rung 1.75 Mbps): DNS 20 ms + TCP and TLS 75 ms + manifest 40 ms + first segment `1.75 × 2 / 10 = 0.35 s` plus RTT ≈ 380 ms (the DRM license, ≈ 120 ms, is fetched in parallel) + decode and first frame 100 ms ≈ **615 ms**, and 735 ms if the license is serial. So start on a low rung, prefetch manifest and first segment on hover, and parallelise the license.
...
Pipeline: contributor to ingest (RTMP is legacy, SRT and WebRTC-based ingest handle loss better), real-time ladder transcode, packaging into segments or parts, CDN, player. Live changes three things. The manifest mutates every segment (short TTL, and the latest segment is requested by every viewer at once, a synchronised herd), the encode cannot use slow presets, and latency becomes a product feature.

| Mode | Delivery | Glass-to-glass (derived) | Scaling |
|---|---|---|---|
| Classic HLS/DASH, 6 s segments | Whole segments over the CDN | Encode 0.5 + ingest 0.2 + transcode 1 + wait for the segment 6 + CDN 0.2 + player buffer of 3 segments (18) ≈ **26 s** | Excellent, plain cacheable files |
| Low-latency HLS or DASH | Parts of ~0.5 s (LL-DASH: CMAF chunks sent with HTTP chunked transfer as they are encoded) published before the segment ends, LL-HLS playlist requests block until the part exists | 0.5 + 0.2 + 1 + 0.5 + 0.2 + player buffer of 3 parts (1.5) = **3.9 s** | Still CDN-cacheable, but needs collapsing on blocked requests |
| WebRTC | Per-viewer connection to an SFU ([22](22_realtime_and_collaboration.md)) | Under 1 s | No shared HTTP cache, so cost per viewer is high and scale is limited |
...
|---|---|---|---|
| Egress | 10M × 4 Mbps | **40 Tbps** (5 TB/s) | No origin can serve this, so the CDN is the product. |
...
- **"How does video get from upload to a viewer?"** Ingest, chunk-parallel encode into a ladder, CMAF plus manifests, origin, CDN, player ABR. Name where cost and latency sit.
...
- [02_networking.md](02_networking.md) — the CDN primer, TLS, DDoS
...
- [27_multi_region_and_global_traffic.md](27_multi_region_and_global_traffic.md) — DNS and anycast steering
