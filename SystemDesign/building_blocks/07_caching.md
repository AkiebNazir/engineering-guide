...
- **LRU** evicts the key unused for longest. It suits recency-skewed traffic and is easy to reason about. Exact LRU needs a linked list reordered on every hit, a shared mutation on the hot path, so real systems approximate it. **Redis** keeps no global list: it samples `maxmemory-samples` keys (default 5) and evicts the one idle longest, which costs a timestamp per key instead of two list pointers. Raising the sample count to 10 gets close to true LRU for more CPU (Redis docs, "Using Redis as an LRU cache"). **CLOCK** arranges entries in a ring with a reference bit that a hit sets and a sweeping hand clears, giving a "second chance" to recently used entries. A hit then writes one bit and reorders nothing, which is why operating systems favour it.
- **LFU** evicts the least frequently used key. It wins when popularity is stable, such as a product catalogue. Pure counts never forget, so yesterday's hot key lingers. Redis 4.0 offers `allkeys-lfu` with a small probabilistic counter that decays (`lfu-decay-time`).
...
- **Scan resistance.** One pass over 10^7 keys that will not be read again flushes a plain LRU of its whole hot set. Defences are segmented LRU or 2Q (a key must be hit twice to be promoted), an admission filter like TinyLFU, or a separate pool for batch traffic.

| Policy | Wins when | Loses when | Cost |
|---|---|---|---|
| LRU (or sampled LRU) | Recency-skewed traffic, simple operations | Scans, floods of one-hit keys | List mutation per hit, or sampling CPU |
| CLOCK | Same as LRU, with cheap concurrent hits | Same as LRU | One bit per entry |
| LFU | Stable popularity, scan-heavy traffic | Popularity shifts, unless counters decay | Counter per key |
...
| Size-aware (GDSF) | Wide spread of object sizes | Uniform sizes, where it reduces to LFU-like | Size tracking |
...
**Decision rule:** plan capacity as data × (1 + per-key overhead + fragmentation) ÷ the fraction of RAM you let the cache use, and alert on evictions per second and the fragmentation ratio as well as hit ratio.
...
**Memory example.** Suppose the working set is 200M keys of 500 B values plus 64 B of key and metadata overhead each: 200M × 564 B = 113 GB. Add slab rounding of about 10% for 124 GB. If you let the cache use 75% of RAM, you need 124 ÷ 0.75 = 165 GB of RAM, so 3 nodes of 64 GB, plus a replica each for availability, 6 in all. At 100k requests/s the primaries see about 33k each, well inside one node's capacity, so **capacity sets the fleet size, not QPS**. Then check the two failure inputs: with an origin sized for 1,000/s, a 99% hit ratio is the contract, and dropping to 95% means 5,000/s. Alert on hit ratio and evictions per second, not on cache CPU.
