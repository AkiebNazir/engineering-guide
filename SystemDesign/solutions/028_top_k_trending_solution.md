...
Served from precomputed results, so reads are key-value lookups (and CDN-cacheable for 30–60 seconds).
...
- **Cost model.** Sketch memory scales with slices, not events, so control slice count and size per slice; log retention is the storage cost, and the CDN absorbs read cost. Report cost per million events.
...
- **Build versus buy.** Build the sketch and merge logic (small) on a managed stream processor and key-value store, and buy the CDN. For small slices, an OLAP store's approximate top-N may be enough.
