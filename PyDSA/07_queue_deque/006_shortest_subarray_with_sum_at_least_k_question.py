...
# ==============================================================================
# TESTS — run:  python 006_shortest_subarray_with_sum_at_least_k_question.py
# ==============================================================================
def _brute(nums, k):
    """O(n^2) reference: prefix-sum-priced scan over all (l, r) pairs."""
    n = len(nums)
    prefix = [0] * (n + 1)
...
def run_tests() -> None:
    ...
    for nums, k in cases:
        ...
        got = sol.shortestSubarray(list(nums), k)
        ok = got == expected
        passed += ok
