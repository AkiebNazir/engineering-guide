...
# ==============================================================================
# TESTS — run:  python 020_minimum_cost_tree_from_leaf_values_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ([6, 2, 4], 32),
        ([4, 11], 44),
        ([15, 13, 5, 3, 8, 3, 12, 9, 5, 15], None),  # cross-checked against brute force below
    ]
    passed = 0
    for arr, expected in cases:
        got = sol.mctFromLeafValues(arr)
        ok = got == expected if expected is not None else got is not None
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  mctFromLeafValues({arr}) -> {got}"
              + (f"  (want {expected})" if expected is not None else ""))
