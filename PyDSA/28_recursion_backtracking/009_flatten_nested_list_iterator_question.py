...
# ------------------------------------------------------------------
# A minimal stand-in for LeetCode's NestedInteger interface, so this
...
def _build(raw):
    """Build NestedInteger objects from plain Python ints/lists, e.g. [[1,1],2]."""
...
# ==============================================================================
# TESTS — run:  python 009_flatten_nested_list_iterator_question.py
# ==============================================================================
def drain(nested_raw):
    it = NestedIterator([_build(x) for x in nested_raw])
    out = []
    while it.hasNext():
        out.append(it.next())
    return out
...
def run_tests() -> None:
    ...
    for nested_raw, expected in cases:
        got = drain(nested_raw)
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  flatten({nested_raw}) -> {got}  (want {expected})")
