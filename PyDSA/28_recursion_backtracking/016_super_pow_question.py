...
# ==============================================================================
# TESTS — run:  python 016_super_pow_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ...
        (2, [0], 1),
        (7, [7], 1288),  # 7^7 = 823543, 823543 % 1337 = 1288
    ]
    passed = 0
    for a, b, expected in cases:
        got = sol.superPow(a, b)
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  superPow({a}, {b}) -> {got}  (want {expected})")
