...
# ==============================================================================
# TESTS — run:  python 012_longest_consecutive_sequence_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ...
        ([], 0),                                  # empty input
        ([1], 1),
        ([1, 2, 0, 1], 3),                        # duplicate inside the run
        ([1, 1, 1, 1], 1),                        # all duplicates
        ([9, 1, 4, 7, 3, -1, 0, 5, 8, -1, 6], 7),  # -1..1 and 3..9
        ([-3, -2, -1, 0], 4),                     # negatives
        ([10, 30, 20], 1),                        # no run at all
        ([1, 2, 3, 100, 101, 102, 103], 4),       # the later run is longer
    ]
    passed = 0
    for nums, expected in cases:
        got = sol.longestConsecutive(list(nums))
        ok = got == expected
        passed += ok
