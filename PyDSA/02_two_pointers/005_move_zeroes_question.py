...
# ==============================================================================
# TESTS — run:  python 005_move_zeroes_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ...
        ([0, 0, 1], [1, 0, 0]),               # zeroes at the front
        ([1, 0, 0], [1, 0, 0]),               # zeroes at the back
        ([4, 2, 4, 0, 0, 3, 0, 5, 1, 0], [4, 2, 4, 3, 5, 1, 0, 0, 0, 0]),
        ([-1, 0, -2, 0], [-1, -2, 0, 0]),     # negatives
        ([0, -1], [-1, 0]),
    ]
    passed = 0
    for nums, expected in cases:
        arr = list(nums)
        ret = sol.moveZeroes(arr)
        ok = arr == expected and ret is None
        passed += ok
        note = "" if ret is None else f"  (returned {ret!r}; must return None)"
        print(f"{'PASS' if ok else 'FAIL'}  {nums} -> {arr} "
