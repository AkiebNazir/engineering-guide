...
# ==============================================================================
# TESTS — run:  python 002_valid_palindrome_ii_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ...
        ("abc", False),              # needs two deletions
        ("a", True),                 # single character
        ("ab", True),                # delete either one
        ("", True),                  # empty
        ("aa", True),
        ("abcda", False),            # two mismatches
        ("deeee", True),             # delete the leading 'd'
        ("eeeed", True),             # mirror of the above
        ("cbbcc", True),             # greedy "delete left" fails here
        ("tebbem", False),
        ("abbba", True),
        ("racecar", True),
        ("racecarx", True),          # delete the trailing 'x'
        ("xracecar", True),
    ...
    for s, expected in cases:
        ...
        ok = got == expected
        passed += ok
        shown = s if len(s) <= 24 else s[:21] + "..."
        print(f"{'PASS' if ok else 'FAIL'}  {shown!r:<28} -> {got} (want {expected})")
