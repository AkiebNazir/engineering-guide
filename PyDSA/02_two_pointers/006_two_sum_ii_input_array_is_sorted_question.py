...
# ==============================================================================
# TESTS — run:  python 006_two_sum_ii_input_array_is_sorted_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ([2, 7, 11, 15], 9, [1, 2]),
        ([2, 3, 4], 6, [1, 3]),
        ([-1, 0], -1, [1, 2]),                 # negatives, minimum length
        ([1, 2], 3, [1, 2]),
        ([0, 0, 3, 4], 0, [1, 2]),             # duplicates, zero target
        ([2, 7, 11, 15], 18, [2, 3]),          # 7 + 11; answer in the middle
        ([5, 25, 75], 100, [2, 3]),            # answer at the end
        ([-10, -8, -2, 1, 2, 5], -18, [1, 2]), # all-negative pair
        ([-3, 3, 4, 90], 0, [1, 2]),           # pair sums to zero
        ([1, 3, 4, 5, 7, 11], 9, [3, 4]),      # both pointers must move
    ]
    passed = 0
    for numbers, target, expected in cases:
        got = sol.twoSum(list(numbers), target)
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  {numbers} target={target} -> {got} "
              f"(want {expected})")
