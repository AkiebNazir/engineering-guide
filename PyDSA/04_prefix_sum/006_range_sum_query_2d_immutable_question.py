...
# ==============================================================================
# TESTS — run:  python 006_range_sum_query_2d_immutable_question.py
# ==============================================================================
def _brute_region(matrix, row1, col1, row2, col2):
    """O(rows*cols) reference: sum the rectangle directly, no precomputation."""
    total = 0
    for i in range(row1, row2 + 1):
        for j in range(col1, col2 + 1):
            total += matrix[i][j]
...
def run_tests() -> None:
    ...
    nm = NumMatrix([row[:] for row in matrix])

    queries = [
        (2, 1, 4, 3, 8),
        (1, 1, 2, 2, 11),
        (1, 2, 2, 4, 12),
        (0, 0, 4, 4, sum(sum(r) for r in matrix)),   # whole matrix
        (0, 0, 0, 0, matrix[0][0]),                   # single cell, top-left
        (4, 4, 4, 4, matrix[4][4]),                   # single cell, bottom-right
        (2, 2, 2, 2, matrix[2][2]),                   # single cell, middle
        (0, 0, 0, 4, sum(matrix[0])),                 # single row
        (0, 0, 4, 0, sum(r[0] for r in matrix)),      # single column
    ]

    passed = 0
    for row1, col1, row2, col2, expected in queries:
        oracle = _brute_region(matrix, row1, col1, row2, col2)
        assert oracle == expected, (
            f"bad test expectation for region ({row1},{col1})-({row2},{col2}): "
            f"oracle says {oracle}, test says {expected}")
        got = nm.sumRegion(row1, col1, row2, col2)
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  region ({row1},{col1})-({row2},{col2}) "
              f"-> {got}  (want {expected})")

    # 1x1 matrix edge case
    nm1 = NumMatrix([[42]])
    ok = nm1.sumRegion(0, 0, 0, 0) == 42
    passed += ok
    print(f"{'PASS' if ok else 'FAIL'}  1x1 matrix -> {nm1.sumRegion(0, 0, 0, 0)}  (want 42)")

    # matrix with negative numbers
    neg = [[1, -2, 3], [-4, 5, -6], [7, -8, 9]]
    nm2 = NumMatrix([row[:] for row in neg])
    oracle = _brute_region(neg, 0, 0, 2, 2)
    got = nm2.sumRegion(0, 0, 2, 2)
    ok = got == oracle
    passed += ok
    print(f"{'PASS' if ok else 'FAIL'}  negatives, whole matrix -> {got}  (want {oracle})")
