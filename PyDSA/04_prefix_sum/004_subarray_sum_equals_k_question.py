...
# ==============================================================================
# TESTS — run:  python 004_subarray_sum_equals_k_question.py
# ==============================================================================
def _brute(nums, k):
    """O(n^2) reference: running inner sum from every start."""
    n = len(nums)
    count = 0
    for i in range(n):
        s = 0
...
def run_tests() -> None:
    ...
    cases = [
        ...
        ([1], 0, 0),
        ([-1, -1, 1], 0, 1),
        ([0, 0, 0], 0, 6),                 # k=0 with zeros: every subarray of zeros counts
        ([-1, -1, -1], -2, 2),             # all negative
        ([3, 4, 7, 2, -3, 1, 4, 2], 7, 4), # mixed, several overlapping answers
        ([1, 2, 3], 100, 0),               # no valid subarray
        ([1, 2, 3], 6, 1),                 # whole array sums to k, starts at index 0
        ([1, 1, 1, 1], 2, 3),              # duplicate prefix sums -> multiple counted subarrays
    ...
    for nums, k, expected in cases:
        assert _brute(nums, k) == expected, (
            f"bad test expectation for {nums!r}, k={k}: "
            f"oracle says {_brute(nums, k)}, test says {expected}")
        got = sol.subarraySum(list(nums), k)
        ok = got == expected
        passed += ok
