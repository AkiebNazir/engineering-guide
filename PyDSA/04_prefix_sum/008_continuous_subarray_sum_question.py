...
# ==============================================================================
# TESTS — run:  python 008_continuous_subarray_sum_question.py
# ==============================================================================
def _brute(nums, k):
    """O(n^2) reference: every subarray of length >= 2, sum it, check."""
    n = len(nums)
    for i in range(n):
        total = nums[i]
...
def run_tests() -> None:
    ...
    cases = [
        ...
        ([23, 2, 6, 4, 7], 6, True),
        ([23, 2, 6, 4, 7], 13, False),
        ([0, 0], 0, True),                  # k=0, adjacent zeros
        ([0, 1, 0], 0, False),              # k=0, no adjacent zeros
        ([5], 5, False),                    # length < 2, no valid subarray
        ([1], 1, False),                    # single element, same reason
        ([1, 0], 1, True),                  # exact length-2 boundary
        ([-1, 2, -1, 3], 2, True),          # negatives
        ([5, 0, 0, 0], 3, True),            # k doesn't divide any element
                                             # alone, but a combination works
        ([1, 2, 3], 5, True),               # 2+3=5
        ([1, 2, 3], 6, True),                # whole-array multiple
    ]
