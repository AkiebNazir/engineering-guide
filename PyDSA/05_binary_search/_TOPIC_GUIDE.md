...
| [003 · First Bad Version](PyDSA/05_binary_search/003_first_bad_version_solution.py) <br>LC 278 · Easy | Oracle, boolean | There is no array: `isBadVersion(v)` *is* the monotone predicate — leftmost True over `1..n`. **Trap:** `lo = 0` (the API is 1-indexed); `hi = mid - 1` when the call returns True. |
