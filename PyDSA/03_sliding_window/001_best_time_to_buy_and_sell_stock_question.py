...
# ==============================================================================
# TESTS — run:  python 001_best_time_to_buy_and_sell_stock_question.py
# ==============================================================================
def _brute(prices):
    """O(n^2) reference: every (buy, sell) pair with buy < sell."""
    best = 0
...
def run_tests() -> None:
    ...
    cases = [
        ([7, 1, 5, 3, 6, 4], 5),
        ([7, 6, 4, 3, 1], 0),          # only falls -> 0, never negative
        ([1], 0),                      # single day: cannot buy AND sell
        ([2, 2, 2], 0),                # flat
        ([1, 2], 1),                   # minimum profitable case
        ([2, 1], 0),
        ([3, 2, 6, 5, 0, 3], 4),       # best pair is NOT the global min/max
        ([2, 4, 1], 2),                # global min is LAST -> useless
        ([1, 4, 2], 3),
        ([0, 0], 0),
        ([5, 4, 3, 2, 1, 100], 99),    # long descent then a spike
        ([100, 1, 2, 3], 2),
    ]
    ...
    for prices, expected in cases:
        ...
        assert _brute(prices) == expected, (
            f"bad test expectation for {prices}: "
            f"oracle says {_brute(prices)}, test says {expected}")
        got = sol.maxProfit(list(prices))
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  {prices} -> {got}  (want {expected})")
