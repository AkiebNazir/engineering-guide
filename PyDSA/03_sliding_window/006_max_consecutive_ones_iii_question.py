...
# ==============================================================================
# TESTS — run:  python 006_max_consecutive_ones_iii_question.py
# ==============================================================================
def _brute(nums, k):
    """O(n^2) reference: extend from every start until the zero budget runs out."""
    best = 0
...
def run_tests() -> None:
    ...
    cases = [
        ...
        ([0, 0, 0, 0], 0, 0),          # k=0 and no ones at all -> 0
        ([1, 1, 1], 0, 3),             # k=0, already all ones
        ([1, 0, 1], 0, 1),             # k=0 -> the longest EXISTING run
        ([0], 1, 1),                   # flip the only element
        ([1], 0, 1),
        ([0, 0, 0], 3, 3),             # k == n, all zeros -> flip everything
        ([0, 0, 0], 5, 3),             # k > number of zeros
        ([1, 0, 0, 1, 1, 0, 1], 2, 5),
        ([0, 1, 1, 1, 0, 0], 1, 4),    # best window starts at index 0
        ([0, 0, 1, 1, 1, 0], 1, 4),    # best window is in the middle
        ([1, 1, 0, 0, 0, 0, 1, 1], 2, 4),
        ([1] * 10, 3, 10),             # no zeros to spend the budget on
    ]

    passed = 0
    for nums, k, expected in cases:
        assert _brute(nums, k) == expected, (
            f"bad test expectation for {nums}, k={k}: "
            f"oracle says {_brute(nums, k)}, test says {expected}")
        got = sol.longestOnes(list(nums), k)
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  k={k:<2} {str(nums):<48} -> {got}  "
              f"(want {expected})")
