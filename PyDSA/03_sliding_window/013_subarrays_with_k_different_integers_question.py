...
# ==============================================================================
# TESTS — run:  python 013_subarrays_with_k_different_integers_question.py
# ==============================================================================
def _brute(nums, k):
    """O(n^2) reference: every subarray, growing a set."""
    count = 0
...
def run_tests() -> None:
    ...
    cases = [
        ...
        ([1, 2, 1, 3, 4], 3, 3),
        ([1], 1, 1),
        ([1, 1, 1, 1], 1, 10),          # all identical: n(n+1)/2 subarrays
        ([1, 2, 3, 4], 1, 4),           # k=1: one per element
        ([1, 2, 3, 4], 4, 1),           # k=n: only the whole array
        ([1, 2, 3, 4], 5, 0),           # k > distinct count -> 0
        ([1, 1, 2, 2], 2, 4),
        ([2, 1, 1, 1, 2], 2, 7),
        ([1, 2, 1, 2, 1], 2, 10),       # only two values, all subarrays len>=2
        ([5, 5, 5], 2, 0),              # never enough distinct values
        ([1, 2], 2, 1),
        ([1, 2, 3, 1, 2, 3], 3, 10),
        ([4, 4, 4, 4], 1, 10),
    ]
    ...
    for nums, k, expected in cases:
        ...
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  k={k:<2} {str(nums):<28} -> {got}  "
              f"(want {expected})")
