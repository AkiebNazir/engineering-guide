...
# ==============================================================================
# TESTS — run:  python 008_minimum_size_subarray_sum_question.py
# ==============================================================================
def _brute(target, nums):
    """O(n^2) reference: every subarray, running sum, keep the shortest."""
    best = float('inf')
    for i in range(len(nums)):
        ...
        for j in range(i, len(nums)):
            ...
            if total >= target:
                best = min(best, j - i + 1)
                break                       # longer starts here only get worse
    return 0 if best == float('inf') else best
...
def run_tests() -> None:
    ...
    cases = [
        (7, [2, 3, 1, 2, 4, 3], 2),
        (4, [1, 4, 4], 1),
        (11, [1, 1, 1, 1, 1, 1, 1, 1], 0),      # impossible -> 0, not inf
        (11, [1, 2, 3, 4, 5], 3),
        (15, [1, 2, 3, 4, 5], 5),               # needs the WHOLE array
        (16, [1, 2, 3, 4, 5], 0),               # one more than the total
        (1, [1], 1),                            # minimal everything
        (2, [1], 0),
        (5, [5], 1),                            # single element hits target
        (5, [1, 1, 1, 1, 10], 1),               # the answer is the LAST element
        (5, [10, 1, 1, 1, 1], 1),               # the answer is the FIRST
        (100, [10] * 10, 10),                   # exactly the whole array
        (3, [1, 1, 1, 1, 1], 3),
        (213, [12, 28, 83, 4, 25, 26, 25, 2, 25, 6], 8),
        (10 ** 9, [10 ** 4] * 10, 0),           # constraint extremes
    ]

    passed = 0
    for target, nums, expected in cases:
        assert _brute(target, nums) == expected, (
            f"bad test expectation for target={target}, {nums}: "
            f"oracle says {_brute(target, nums)}, test says {expected}")
        got = sol.minSubArrayLen(target, list(nums))
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  target={target:<11} "
              f"{str(nums):<42} -> {got}  (want {expected})")
