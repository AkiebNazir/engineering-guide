...
# ==============================================================================
# TESTS — run:  python 010_permutation_in_string_question.py
# ==============================================================================
def _brute(s1, s2):
    """O(n*k) reference: sort every window of length len(s1) and compare."""
    k = len(s1)
    target = sorted(s1)
    return any(sorted(s2[i:i + k]) == target for i in range(len(s2) - k + 1))
...
def run_tests() -> None:
    ...
    cases = [
        ("ab", "eidbaooo", True),
        ("ab", "eidboaoo", False),
        ("a", "a", True),                    # minimal true
        ("a", "b", False),
        ("abc", "ab", False),                # s1 LONGER than s2
        ("adc", "dcda", True),
        ("hello", "ooolleoooleh", False),    # right letters, never contiguous
        ("abc", "ccccbbbbaaaa", False),      # all letters present, never together
        ("ab", "ab", True),                  # whole string
        ("ab", "ba", True),                  # exact permutation
        ("aab", "aba", True),
        ("aab", "abb", False),               # MULTIPLICITY matters: two a's needed
        ("abb", "aab", False),               # the mirror of the above
        ("ab", "eidbao", True),              # match at the very END
        ("ab", "baeido", True),              # match at the very START
        ("xy", "abcdefgh", False),           # no letters in common at all
        ("aaa", "aaaa", True),
        ("aaaa", "aaa", False),
    ]
    ...
    for s1, s2, expected in cases:
        assert _brute(s1, s2) == expected, (
            f"bad test expectation for {s1!r}, {s2!r}: "
            f"oracle says {_brute(s1, s2)}, test says {expected}")
        got = sol.checkInclusion(s1, s2)
        ok = got == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  s1={s1!r:<8} s2={s2!r:<16} -> {got}  "
              f"(want {expected})")
