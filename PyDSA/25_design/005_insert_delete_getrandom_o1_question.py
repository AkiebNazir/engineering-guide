...
# ==============================================================================
# TESTS — run:  python 005_insert_delete_getrandom_o1_question.py
# ==============================================================================
def run_tests() -> None:
    all_ok = True
    ...
    ok = rs.getRandom() == 2  # only element left
    all_ok &= ok
    print(f"{'PASS' if ok else 'FAIL'}  getRandom() with a single element returns that element")

    # getRandom must eventually return every inserted element (basic sanity,
    # not a rigorous uniformity check — see the solution file for that).
    rs2 = RandomizedSet()
    for v in (10, 20, 30):
        rs2.insert(v)
    seen = {rs2.getRandom() for _ in range(200)}
    ok = seen == {10, 20, 30}
    all_ok &= ok
    print(f"{'PASS' if ok else 'FAIL'}  getRandom() over 200 draws covers all inserted elements  seen={seen}")
