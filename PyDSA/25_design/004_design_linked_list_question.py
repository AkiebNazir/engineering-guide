...
# ==============================================================================
# TESTS — run:  python 004_design_linked_list_question.py
# ==============================================================================
def run_tests() -> None:
    all_ok = True
    ...
    # Out-of-range get on an empty list.
    empty = MyLinkedList()
    ok = empty.get(0) == -1
    all_ok &= ok
    print(f"{'PASS' if ok else 'FAIL'}  get(0) on empty list returns -1")

    # addAtIndex(size, val) appends at tail; addAtIndex(> size, val) no-ops.
    ll2 = MyLinkedList()
    ll2.addAtHead(10)
    ll2.addAtIndex(1, 20)  # index == size (1) -> append
    ll2.addAtIndex(99, 30)  # index > size -> no-op
    ok = ll2.get(0) == 10 and ll2.get(1) == 20 and ll2.get(2) == -1
    all_ok &= ok
    print(f"{'PASS' if ok else 'FAIL'}  addAtIndex boundary cases (== size appends, > size no-ops)")
