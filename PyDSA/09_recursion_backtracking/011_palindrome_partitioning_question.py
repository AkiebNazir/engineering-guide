...
# ==============================================================================
# TESTS — run:  python 011_palindrome_partitioning_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
    ...
    for s_input, expected in cases:
        got = sol.partition(s_input)
        # Sort both lists of lists for order-agnostic comparison
        got_sorted = sorted([sorted(p) for p in got]) if got else []
        exp_sorted = sorted([sorted(p) for p in expected])
        
        ok = got_sorted == exp_sorted
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  s={s_input!r}")
        if not ok:
