...
# ==============================================================================
# TESTS — run:  python 010_letter_combinations_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    cases = [
        ...
        ("",   []),
        ("2",  sorted(["a","b","c"])),
        ("79", sorted(["pw","px","py","pz","qw","qx","qy","qz",
                        "rw","rx","ry","rz","sw","sx","sy","sz"])),
    ]
    passed = 0
    for digits, expected in cases:
        got = sol.letterCombinations(digits)
        got_sorted = sorted(got) if got else []
        ok = got_sorted == expected
        passed += ok
        print(f"{'PASS' if ok else 'FAIL'}  digits={digits!r}")
        if not ok:
            print(f"    Expected: {expected}")
            print(f"    Got:      {got_sorted}")
