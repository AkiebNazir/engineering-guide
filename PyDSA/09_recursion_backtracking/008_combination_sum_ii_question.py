...
# ==============================================================================
# TESTS — run:  python 008_combination_sum_ii_question.py
# ==============================================================================
def _oracle(candidates: List[int], target: int) -> List[List[int]]:
    """Independent reference: enumerate every SUBSET of indices with a
    bitmask, keep those summing to target, dedupe by sorted tuple. O(2^n),
    so only usable on small inputs — which is exactly what the cases are."""
    n = len(candidates)
    seen = set()
...
def run_tests() -> None:
    ...
    for candidates, target in cases:
        expected = sorted(sorted(c) for c in _oracle(candidates, target))
        got = sol.combinationSum2(list(candidates), target)
