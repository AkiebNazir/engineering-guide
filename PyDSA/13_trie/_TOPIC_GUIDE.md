...
2. **Autocomplete / "all words with this prefix"** — walk to the prefix node
   once, then DFS the subtree below it; a hash set has no notion of "subtree."
3. **Multi-pattern search over a fixed grid/text** (Word Search II, 006) —
   ...
   single grid DFS. Checking each of N words independently against the grid
...
4. **Wildcard search** (`.` matches any character, 002) — a hash set has no
   way to skip the unknown character; a trie DFS branches over all children
...
## Part 4 · Wildcard Search — DFS Over the Trie
...
stored set (005 Maximum XOR of Two Numbers), greedily walk the *opposite*
...
5. In wildcard search, forgetting the `i == len(word)` base case check
   ...
   bug as #1, in DFS form).
6. Off-by-one in the bit trie: iterating bits from the wrong end (must walk
   most-significant-bit first for the greedy-XOR trick in Part 7 to be
   correct).
...
- "How would you support autocomplete (top-k suggestions by frequency)?" —
  augment nodes with a counter (Part 6 pattern) and DFS the subtree collecting
...
- **Topic 09 (Recursion & Backtracking)** — Word Search II (006) reuses the
  ...
  grid in lockstep.
- **Topic 20 (Bit Manipulation)** — the binary/XOR trie (Part 7) is the
...
(`node.pop(END)` — which also dedupes results), and when a node has no words and no children left after its DFS,
...
On a random 6×6 grid over `{a, b}` with 200 words of length 3–8, both versions return the *same 98 words*, but the DFS
...
|---|---|---|
| **Radix / Patricia tree** | Collapse every single-child chain into one edge labelled with a substring | Far fewer nodes on sparse data; IP routing tables, some in-memory databases. |
...
| **Aho–Corasick** | A trie plus *failure links* (like KMP's table, topic 23) | Find *all* occurrences of *many* patterns in one pass over a text: O(text + matches). |
| **Suffix trie / tree / array** | Index every *suffix* of a text | Substring search, longest repeated substring, bioinformatics. |
| **Bit trie** | 2 children per node, keys are integers | Max XOR (Part 7), IP longest-prefix match. |
...
| "Top-k suggestions." | Precompute the top-k per node (13.4), or DFS the subtree with a heap. |
...
| [002 · Design Add and Search Words Data Structure](PyDSA/13_trie/002_design_add_and_search_words_data_structure_solution.py) <br>LC 211 · Medium | Wildcard search by DFS | `addWord` is 001's `insert`; `search` becomes a DFS where `.` tries every live child. **Trap:** no `node is None` guard at the top of the recursion; checking `is_word` before confirming `i == len(word)`. |
...
| [005 · Maximum XOR of Two Numbers in an Array](PyDSA/13_trie/005_maximum_xor_of_two_numbers_in_an_array_solution.py) <br>LC 421 · Medium | A bit trie | A fixed-depth trie over the bits of each number, most significant first; for each `x` greedily walk the *opposite* bit to maximise the XOR. **Trap:** not padding to a fixed bit width (misaligned bits); walking least-significant first (the greedy argument only holds MSB-first). |
| [006 · Word Search II](PyDSA/13_trie/006_word_search_ii_solution.py) <br>LC 212 · Hard | Trie + grid backtracking | One trie from all words, then one DFS over the grid walking trie and grid in lockstep; stop descending the moment the trie has no such child. **Trap:** forgetting to restore `board[r][c]` after the call; a separate `visited` set instead of the in-place sentinel. |
| [007 · Search Suggestions System](PyDSA/13_trie/007_search_suggestions_system_solution.py) <br>LC 1268 · Medium | Two designs for autocomplete | A sorted list plus `bisect_left` (prefix words are one contiguous block), or a trie with a DFS. **Trap:** taking `products[i:i+3]` without the `startswith` filter (`"b"` sneaks into `"ap"`); not sorting first. |
...
- [ ] Trace a wildcard DFS search including the fan-out at `.`
- [ ] Explain the bit-trie / max-XOR greedy walk
