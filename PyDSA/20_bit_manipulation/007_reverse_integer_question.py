...
def run_tests() -> None:
    ...
    cases = [
        ...
        (120, 21),
        (0, 0),
        (1534236469, 0),   # reverses to 9646324351, overflows INT32_MAX
        (-2147483648, 0),  # reverses to -8463847412, overflows INT32_MIN
        (1463847412, 2147483641),  # a legitimate near-boundary success
    ]
    for x, want in cases:
        got = sol.reverse(x)
        ok = got == want
        all_ok &= ok
        print(f"{'PASS' if ok else 'FAIL'}  x={x:>12}  -> {got:>12}  (want {want})")
