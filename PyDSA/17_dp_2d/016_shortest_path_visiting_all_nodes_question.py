...
# ==============================================================================
# TESTS — run:  python 016_shortest_path_visiting_all_nodes_question.py
# ==============================================================================
def run_tests() -> None:
    cases = [
        ([[1, 2, 3], [0], [0], [0]], 4),
    ...
    for graph, want in cases:
        got = Solution().shortestPathLength(graph)
        ok = got == want
        all_ok &= ok
        print(f"{'PASS' if ok else 'FAIL'}  graph={graph}  got={got}  want={want}")
