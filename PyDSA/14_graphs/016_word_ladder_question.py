...
def run_tests() -> None:
    ...
    cases = [
        ...
        ("same", "same", ["same"], 0),  # not reachable per constraints normally,
                                          # but guard against beginWord==endWord
    ]
    for begin, end, words, expected in cases:
        # skip the degenerate begin==end case's guard note; constraints say
        # beginWord != endWord always holds, this case is just a safety probe
        if begin == end:
            continue
