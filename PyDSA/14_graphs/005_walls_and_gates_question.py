...
class Solution:
    def solve(self, rooms: List[List[int]]) -> None:
        """Modify rooms in-place. No return value (per LC signature)."""
        # YOUR CODE HERE
        pass
...
def run_tests() -> None:
    ...
    cases = [
        ...
        ([[-1]], [[-1]]),
        ([[0]], [[0]]),
        ([[INF]], [[INF]]),                       # no gate at all -> stays INF
        (
            [[0, INF], [INF, INF]],
            [[0, 1], [1, 2]],
        ),
        (
            [[-1, -1, -1], [-1, 0, -1], [-1, -1, -1]],   # gate walled in
            [[-1, -1, -1], [-1, 0, -1], [-1, -1, -1]],
        ),
    ]
    ...
    for i, (grid, expected) in enumerate(cases):
        ...
        ok = working == expected
        all_ok &= ok
        print(f"{'PASS' if ok else 'FAIL'}  case {i}: -> {working}  (want {expected})")
