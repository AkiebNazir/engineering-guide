...
# ==============================================================================
# TEST HELPERS — validate ANY correct order, not just one fixed expected list
# ==============================================================================
...
# ==============================================================================
# TESTS — run:  python 012_course_schedule_ii_question.py
# ==============================================================================
def run_tests() -> None:
    sol = Solution()
    all_ok = True
    ...
    for n, prereqs, should_exist in cases:
        got = sol.findOrder(n, [row[:] for row in prereqs])
        if should_exist:
            ok = isinstance(got, list) and is_valid_order(got, n, prereqs)
        else:
            ok = got == []
        all_ok &= ok
        print(f"{'PASS' if ok else 'FAIL'}  numCourses={n} prereqs={prereqs!r:<36} "
              f"-> {got}")
